...
echo "##### 첫 번째 노드에 azname 추가\n"
export FIRST_NODE_NAME=$(kubectl get nodes -o json | jq -r '.items[0].metadata.name')
kubectl label nodes ${FIRST_NODE_NAME} azname=az1

echo "##### affinity 1 : 꼬옥 azname이 az1 또는 az2인 노드들\n"
echo "##### affinity 2 : 우선 another-node-label-key=another-node-label-value 노드들... 없으면... 암데나..\n"
